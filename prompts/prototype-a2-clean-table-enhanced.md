# Prompt A-2: "Clean Table Enhanced" — ExifCleaner UI Prototype

> Copy-paste this entire file into Claude (artifacts) or v0 to generate an interactive mockup.
> This is a revision of Prompt A, adding: progress bar, metadata expansion, file reveal, scroll behavior, and folder structure.
...
2. Clickable rows that expand to show metadata details
3. A "reveal in Finder" action on hover
...
16px  — standard padding (cards, buttons, expansion panel)
24px  — section gaps
32px  — large gaps
```
...
Fast:        200ms ease-out     (row entrance, expansion panel, popover)
...
All files processed. Progress bar turns green briefly, then fades. Mix of success and error.
...
│    ✓ IMG_0002.jpg        31            0                │
│    ✓ IMG_0003.jpg        18            0                │
│  ▶ ✓ IMG_0004.jpg        27            0                │  ← expanded row (clicked)
│  ┌──────────────────────────────────────────────────┐   │
│  │  Camera Make      Canon EOS R5      → (removed)  │   │  ← inline metadata panel
│  │  GPS Latitude     37.7749° N        → (removed)  │   │
│  │  GPS Longitude    -122.4194° W      → (removed)  │   │
│  │  Orientation      Horizontal        → Horizontal  │   │  ← preserved (green)
│  │  ... 23 more tags                                 │   │
...
│    ...                                                  │
│  📁 Documents/ (3 files)                                │
│    ✗ locked_file.pdf      —        Can't read           │  ← error row
│    ...                                                  │
├─────────────────────────────────────────────────────────┤
│                                              Clear ⌘K   │
└─────────────────────────────────────────────────────────┘
...
- Row background highlights to Neutral 200 (`#EEF0F3` light / `#252528` dark)
...
### Click Row → Inline Metadata Expansion

Clicking a **completed** row (success or error) expands an inline panel below it:

```
│  ✓ IMG_0004.jpg        27            0                │  ← clicked row (highlighted bg)
│  ┌──────────────────────────────────────────────────┐ │
│  │                                                   │ │
│  │  Tag                    Before         After      │ │  ← monospace, 12px
│  │  ─────────────────────  ───────────    ───────    │ │
│  │  Camera Make            Canon EOS R5   (removed)  │ │  ← strikethrough on value
│  │  Camera Model           EOS R5         (removed)  │ │
│  │  GPS Latitude           37.7749° N     (removed)  │ │
│  │  GPS Longitude          -122.4194° W   (removed)  │ │
│  │  Date/Time Original     2025-08-14     (removed)  │ │
│  │  Software               Lightroom 7.2  (removed)  │ │
│  │  Color Space            sRGB           sRGB       │ │  ← preserved (green text)
│  │  Orientation            Horizontal     Horizontal │ │  ← preserved (green text)
│  │                                                   │ │
│  │  27 tags examined · 25 removed · 2 preserved      │ │  ← summary, micro text
│  │                                                   │ │
│  └──────────────────────────────────────────────────┘ │
```

**Expansion panel design**:
- Background: surface color (white / dark surface), subtle top border
- Padding: 16px
- Font: monospace for tag alignment, 12px
- "Before" column: value in strikethrough + strikethrough color if removed
- "After" column: "(removed)" in muted text, OR the preserved value in green
- Show top 8 tags by default. If more, show "... N more tags" link that expands the full list.
- Summary line at bottom: "X tags examined · Y removed · Z preserved" in micro text
- **Animation**: Height expands from 0 (200ms ease-out). Content fades in (150ms, 50ms delay).
- Click the same row again to collapse (200ms collapse animation)
- Clicking a different row collapses the current one and expands the new one

**Only completed rows are expandable**. Processing/pending rows are not clickable (cursor: default).

### Error Row Expansion

Clicking a failed row shows a different panel:

```
│  ✗ locked_file.pdf      —        Can't read           │
│  ┌──────────────────────────────────────────────────┐ │
│  │                                                   │ │
│  │  Error: Unable to read file                       │ │  ← error color, 14px
│  │  ExifTool: "File format error"                    │ │  ← mono, muted
│  │                                                   │ │
│  │  The file may be corrupted or use an              │ │  ← body text, helpful
│  │  unsupported format.                              │ │
│  │                                                   │ │
│  └──────────────────────────────────────────────────┘ │
```
...
7. **Click any row** → metadata expansion.
...
8. **Click a completed row** → metadata expansion.
...
t=10.5s   Auto-expand one row (IMG_0004.jpg) to showcase metadata panel.
```
...
### Metadata Detail Data (for expanded rows)

**IMG_0004.jpg** (auto-expanded at end of demo):
```
Tag                    Before Value           After
─────────────────────  ─────────────────────  ──────────
...
Color Space            sRGB                   sRGB          ← preserved (green)
Orientation            Horizontal             Horizontal    ← preserved (green)
... 17 more tags
...
**damaged_photo.jpg** (error expansion):
```
...
**locked_file.pdf** (error expansion):
```
...
- **Metadata expansion must feel informative** — monospace alignment, clear strikethrough for removed tags, green for preserved tags. It should build trust: "I can see exactly what was removed."
- **The reveal icon must feel discoverable but not cluttered** — it only appears on hover, and it's small. Users who need it find it. Users who don't never notice it.
...
- **Dark mode must look intentionally designed** — the gold progress bar glows against the dark background, the metadata panel is a darker surface card, the green/red status colors pop.
- **Folder indentation must be minimal** — just 12px. Enough to see the hierarchy, not enough to waste horizontal space.

**Build this prototype now.** Generate one React component with all states, interactions, scroll behavior, folder groups, metadata expansion, progress bar, and the 38-file dataset. Auto-start the folder flow on load.
